...
void Host_Version_f (void)
{
	...
	Con_Printf ("SilverSrc Version %1.2f.%d\n", SILVERSRC_VERSION, SILVERSRC_VER_PATCH);
...
for (int i = 1; i < com_argc; i++) {
    if (strcmp(com_argv[i], "-basedir") == 0 && i + 1 < com_argc) {
        Con_Printf("\n========= Goldsrc Game (%s) Initialized =========\n\n", com_argv[i+1]);
        Con_Printf("If running a quake mod, then it is not officially supported by SilverSrc And Its forks.\n");
        break;
    }
}
