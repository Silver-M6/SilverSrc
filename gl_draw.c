...
// draw.c -- 2d drawing
//  ̶T̶h̶i̶s̶ ̶i̶s̶ ̶m̶o̶d̶i̶f̶i̶e̶d̶ ̶t̶o̶ ̶a̶d̶d̶ ̶t̶t̶f̶ ̶s̶u̶p̶p̶o̶r̶t̶ ̶i̶n̶ ̶c̶o̶n̶s̶o̶l̶e̶,̶
//  FYI: this is no longer planned due to
//  SilverSrc loading the default quake conchars in memory.
#include "quakedef.h"
#include "conchars.h"
#define concharsowner "conchars.h"
// #include <SDL_ttf.h> - USELESS
...
qpic_t *Draw_PicFromWad (const char *name)
{
	...
	else
	{
		...
		q_snprintf (texturename, sizeof(texturename), "%s:%s", WADFILE_H, name); //johnfitz

		offset = (src_offset_t)p - (src_offset_t)wad_base + sizeof(int)*2; //johnfitz

		gl.gltexture = TexMgr_LoadImage (NULL, texturename, p->width, p->height, SRC_INDEXED, p->data, WADFILE_H,
...
Changed to load the conchars in memory -- SilverSrc Developers (Replaced with the normal source code because of wad3 being implemented in
the feat-2 branch)
===============
*/
...
void Draw_LoadPics (void)
{
	...
	char_texture = TexMgr_LoadImage (NULL, WADFILE_H":conchars", 128, 128, SRC_INDEXED, data,
		WADFILE_H, offset, TEXPREF_ALPHA | TEXPREF_NEAREST | TEXPREF_NOPICMIP | TEXPREF_CONCHARS);

// Disabled because of WAD3 Support, you can still enable it if you want to.
/*    byte *data = conchars;
    int size = conchars_len;
    if (!data)
    {
        Sys_Error ("conchars walked away. (conchars wasnt found in memory)");
        return;
    }

//  this is garbage code but works
    int tex_w = 128, tex_h = 128;
    if (conchars_len == 256*128) { tex_w = 256; tex_h = 128; }
    else if (conchars_len == 128*256) { tex_w = 128; tex_h = 256; }

    if (size != (256 * 128))
    {
        Sys_Error("Conchars is too fat (size is bigger than 128x128 or 256x128 or 128x256)");
        return;
    }

        char_texture = TexMgr_LoadImage(NULL, "conchars", tex_w, tex_h, SRC_INDEXED, data, concharsowner, 0, TEXPREF_ALPHA | TEXPREF_NEAREST | TEXPREF_NOPICMIP | TEXPREF_CONCHARS);
    if (char_texture)
{
    char_texture->width = 128;
    char_texture->height = 256;
}
else
{
    Sys_Error("Conchars went to eat a bowl of cereal, maybe wait? (aka something in conchars got fucked)");
} */

// down below this code breaks the compiler, its probably because of the new conchars up
/*	byte		*data;
	src_offset_t	offset;
...
Modified to fix Conchars, -- silversrc developers
================
*/
void Draw_CharacterQuad (int x, int y, char num)
{
	int				row, col;
	float			frow, fcol, size_h = 0.0312, size_v = 0.0625, size;
	row = num>>4;
	col = num&31;
	...
	glVertex2f (x, y);
	glTexCoord2f (fcol + size_h, frow);
	glVertex2f (x+8, y);
	glTexCoord2f (fcol + size_h, frow + size_v);
	glVertex2f (x+8, y+8);
...
void GL_SetCanvas (canvastype newcanvas)
{
	...
	case CANVAS_WARPIMAGE:
		glOrtho (0, 256, 0, 128, -99999, 99999);
