...
void M_Help_Draw (void)
{
    ...
    if (!printedhelp)
	{
	Con_Printf("help - Displays this command\n");
	printedhelp = true;
	printedhelp = false;
	}
}
...
void M_Quit_Draw (void) //johnfitz -- modified for new quit message
{
	...
	sprintf(msg1, "QuakeSpasm 0.92.0 (c) Ozkan Sezer, Eric Wasylishen & others \n");
	sprintf(msg1, "SilverSrc %1.2f.%d", (float)QUAKESPASM_VERSION, QUAKESPASM_VER_PATCH);
...
void M_Init (void)
{
	...
	Cmd_AddCommand ("help", M_Help_Draw);
